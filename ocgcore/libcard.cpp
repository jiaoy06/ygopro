...
int32 scriptlib::card_is_synchro_summonable(lua_State *L) {
	...
	// TODO
	if (!(pcard->data.type & (TYPE_SYNCHRO + TYPE_DARKSYNCHRO)))
		return 0;
	card* tuner = 0;
	group* mg = 0;
	if(!lua_isnil(L, 2)) {
...
int32 scriptlib::card_is_able_to_deck_or_extra_as_cost(lua_State *L) {
	...
	card* pcard = *(card**) lua_touserdata(L, 1);
	uint32 p = pcard->pduel->game_field->core.reason_player;
	int32 val = (pcard->data.type & (TYPE_FUSION + TYPE_SYNCHRO + TYPE_XYZ + TYPE_LINK + TYPE_DARKSYNCHRO)) ? pcard->is_capable_cost_to_extra(p) : pcard->is_capable_cost_to_deck(p);
	if(val)
		lua_pushboolean(L, 1);
	else
		lua_pushboolean(L, 0);
